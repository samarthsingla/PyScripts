...
sensitivity = 50 #more means less
averager = 0.01 #
...
monitor_res = (display_info.current_w//2, display_info.current_h//2)
...
def renderText(txt):
    global w, h
    image=generateText(txt,fonts.winorlose,45)
    rect=image.get_rect()
    rect.center=(w / 2,h / 2)
...
def smoothit(values, previous):
    global averager

    diff = values[2] - 16384
    print(diff)
    if abs(diff) > 900:
        #previous[i]+=diff * averager
        return values[0] / abs(values[0])
    else:
        return False
...
try:
    while running and not won:
        ...
        #Orientation Detection Start
        try:
            data=ser.readline().decode("utf-8").split(',')
            vals=[int(data[0]),int(data[1]),int(data[2])] #Acceleration values on the three axes

            diff = smoothit(vals, prev)

            if diff:
                for i in range(weightage(abs(diff))):
                    if diff < 0:
                        scroll(1)
                    else:
                        scroll(0)
            ...
            pass
        # if diff:
        #     for i in range(diff//5000):
        #         print("DEBUGGGG")
        #         scroll(diff + abs(diff))

        # except:
        #     print("Comm: DATA dropped unexpectedly.")
